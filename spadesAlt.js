...
one more thing  keeping everything happy   git made other changes */
...
function spadesMain () {
//Global Variable Declarations for the Game of Spades
	
	//Holds and tracks the essential game stats for determining the winner and
	//the outcome of the game
	function spadesGame (nsScore, ewScore, d, l) {
		this.ns = nsScore;
		this.ew = ewScore;
		this.dealer = d;
		this.leader = l;
	...
	//Holds and tracks the essential round stats for determining the winner
	//and outcome for the round
	function spadesRound () {
		this.books = [];
		this.nsBid = 0;
		this.ewBid = 0;
		this.nsScore = 0;
	...
	//Holds and tracks the essential book stats for determining the winner and
	//outcome for the book
	function spadesBook () {
	...
	//Creates a standard deck of cards 13 values x 4 suits
	//Aces are high, Spades are trump
	//Returns the array
	function spadesDeck () {
		deck = [];
		for (var i = 0; i < 52; i++) {
	...
	//Holds and tracks the player's name, hand, bid and
	//how many books they took for scoring
	function player (name, seat) {
		...
		this.bid = 0;
		this.books = 0;
		showHand = function () {display()}
		}
	}

	//Clears the table so a new hand may be started on a fresh surface
	function clearTable () {
		clearPile();
		clearMessages();
		clearHands();
	...
	//Clears players hands leaving only their names
	function clearHands () {
		for (i = 0; i < 4; i++) {
